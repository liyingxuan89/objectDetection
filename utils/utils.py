...
def parse_violation_res(res_json, scenario="sensor", parameters=None):
    ...
    if scenario == "beam":
        ...
        total = len(df)
        one_half = int(total / 2)
        p1 = df.iloc[:one_half, :]
        p2 = df.iloc[one_half:, :]
        case1 = p1[(p1["试探员"] == 1) & (p1["工人"] == 0)]
        if len(case1) > 100:
            res["violation"] = "yes"
            res["单个试探员作业"] = 1
        ...
        case3 = df[(df["液压机"] == 0) & (df["前探梁"] == 1)]
        if case3.sum()["前探梁满足"] < 360:
            res["violation"] = "yes"
            res["未安装足够前探梁"] = 0.8
            prove2 = list(p2[p2["前探梁"] == 1].index)[::10]
            res["证据"] += prove2
        res["证据"].sort()
        return json.dumps(res)
